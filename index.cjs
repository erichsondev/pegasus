...
// ==========================================
// ROTAS DE USUÁRIOS (Mapeadas de api.ts)
// ==========================================
...
// ==========================================
// ROTAS DE RESUMO (Home Dashboard)
// ==========================================
...
app.get('/api/resumo', autenticar, async (req, res) => {
  ...
  try {
    // Cálculo simplificado para devolver a estrutura esperada pelo resumo
...
// Rota para Sincronizar Agenda (Limpeza de Fantasmas do Matriz.tsx)
app.post('/api/manutencao/sincronizar-agenda', autenticar, async (req, res) => {
  ...
  try {
    ...
    // 3. Recria as previsões para o mês atual e o próximo
    const hoje = new Date();
    const mesesAlvo = [hoje.getMonth(), hoje.getMonth() + 1]; // Mês atual e próximo
    const anoAtual = hoje.getFullYear();

    for (const item of fixos.rows) {
      for (const mesOffset of mesesAlvo) {
        // Ajusta ano se virar o mês para janeiro
        const ano = mesOffset > 11 ? anoAtual + 1 : anoAtual;
        const mes = mesOffset > 11 ? 0 : mesOffset;
        
        // Monta a data alvo baseado no dia_do_mes configurado
        // Adiciona padStart para garantir 2 dígitos
        ...
        if (dataAlvoObj >= inicioVigencia && dataAlvoObj <= fimVigencia) {
          await client.query(
