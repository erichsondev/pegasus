...
app.post('/api/usuarios/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { id: user.rows[0].id, email: user.rows[0].email, nome: user.rows[0].nome }, 
      JWT_SECRET, 
      { expiresIn: '7d' }
    );
...
// ==========================================
// 2. ROTAS DE RESUMO (Home Dashboard)
// ==========================================
...
// A ROTA ESQUECIDA: Reverte transação concluída de volta para prevista
...
// ==========================================
// 5. ROTAS DE LANÇAMENTOS FIXOS & MANUTENÇÃO
// ==========================================
...
app.post('/api/lancamentos-fixos', autenticar, async (req, res) => {
  ...
  try {
    ...
    // Projeção Contínua Baseada na Vigência Real (Mês Absoluto)
    const hoje = new Date();
    const inicioVig = new Date(data_inicio);
    // Se não houver data_fim, projeta com horizonte longo seguro (5 anos) para garantir o comportamento ilimitado contínuo
    const fimVig = data_fim ? new Date(data_fim) : new Date(hoje.getFullYear() + 5, 11, 31);
    
    const absInicio = inicioVig.getFullYear() * 12 + inicioVig.getMonth();
    const absFim = fimVig.getFullYear() * 12 + fimVig.getMonth();

    let pointer = new Date(inicioVig.getFullYear(), inicioVig.getMonth(), 1);

    while (pointer <= fimVig) {
      const ano = pointer.getFullYear();
      const mes = pointer.getMonth();
      const absAlvo = ano * 12 + mes;

      if (absAlvo >= absInicio && absAlvo <= absFim) {
        const dataStr = `${ano}-${String(mes + 1).padStart(2, '0')}-${String(dia_do_mes).padStart(2, '0')}`;
        await client.query(
          "INSERT INTO transacoes (usuario_id, categoria_id, descricao, valor, tipo, status, data) VALUES ($1, $2, $3, $4, $5, 'previsto', $6)",
          [req.user.id, categoria_id || null, descricao, valor, tipo, dataStr]
        );
      }
...
// Rota para Sincronizar Agenda (Limpeza e reconstrução das projeções pendentes)
app.post('/api/manutencao/sincronizar-agenda', autenticar, async (req, res) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    // 1. Remove todas as previsões futuras que ainda não foram pagas
    ...
    // 3. Recria as previsões futuras alinhadas perfeitamente às vigências
    const hoje = new Date();
    const hojeStr = hoje.toISOString().split('T')[0];

    for (const item of fixos.rows) {
      const inicioVig = new Date(item.data_inicio);
      const fimVig = item.data_fim ? new Date(item.data_fim) : new Date(hoje.getFullYear() + 5, 11, 31);
      
      const absInicio = inicioVig.getFullYear() * 12 + inicioVig.getMonth();
      const absFim = fimVig.getFullYear() * 12 + fimVig.getMonth();

      // Inicia a verificação a partir do mês atual ou da data de início (o que for mais recente)
      let pointer = new Date(hoje.getFullYear(), hoje.getMonth(), 1);
      if (inicioVig < pointer) {
        pointer = new Date(inicioVig.getFullYear(), inicioVig.getMonth(), 1);
      }
      ...
      while (pointer <= fimVig) {
        ...
        if (absAlvo >= absInicio && absAlvo <= absFim) {
          const dataStr = `${ano}-${String(mes + 1).padStart(2, '0')}-${String(item.dia_do_mes).padStart(2, '0')}`;
          
          // Garante a inserção apenas se a data de cobrança for >= hoje (evitando recriar pendências no passado)
          if (dataStr >= hojeStr) {
            await client.query(
              "INSERT INTO transacoes (usuario_id, categoria_id, descricao, valor, tipo, status, data) VALUES ($1, $2, $3, $4, $5, 'previsto', $6)",
              [req.user.id, item.categoria_id, item.descricao, item.valor, item.tipo, dataStr]
            );
          }
        }
        pointer.setMonth(pointer.getMonth() + 1);
